import os, re, argparse, logging, getpass, string
# from sys import platform

live_folder_prefix = 'Live '
live_folder_regex = "{prefix}(\d*)\.*(\d*)\.*(\d*)".format(prefix=live_folder_prefix)
...
def get_live_version_folder(root_pref_folder, live_version=''):
    live_folders = []
    for name in os.listdir(root_pref_folder):
        if (
            os.path.isdir(os.path.join(root_pref_folder, name))
            and live_folder_prefix in name
        ):
            live_folders.append(name)
    if not live_folders:
        logging.exception('There are no Ableton Live preferences folders in "{folder}" that match format "{format}"'.format(folder=root_pref_folder, format=live_folder_regex))
        return
    # if a live version was specified via command args look for it's corresponding folder
    if live_version:
        live_folder = '{prefix}{version}'.format(prefix=live_folder_prefix, version=live_version)
        if live_folder not in live_folders:
            logging.exception('Ableton Live preferences folder for version "{version}" was not found"'.format(version=live_version))
            return
        return live_folders[live_folders.index(live_folder)]
    # otherwise use the latest version in the list of folders
...
def get_live_preferences_filepath(live_version=''):
    ...
    return '{root_pref_folder}{live_folder}{pref_filename}'.format(
...
def get_recent_set(live_version=''):
    ...
    if not os.path.exists(prefs_filepath):
        logging.exception('Attempted to use preferences file "{filepath}" but the file does not exist'.format(filepath=prefs_filepath))
        return
    ...
    for chunk in filtered_prefs_content.split('FileRef'):
        if chunk.find('.als') > -1:
            ...
            break
    if not recent_set:
        logging.exception('Recent set could not be found in preferences file "{filepath}"'.format(filepath=prefs_filepath))
        return
    return recent_set

def main():
    parser = argparse.ArgumentParser(description='Display the filepath of the most recent Ableton Live set by parsing Ableton\'s Preferences.cfg.')
    parser.add_argument("-v", "--live_version", help = "Preferred Live version to use (defaults to the latest version if omitted)", required = False, default = "")
