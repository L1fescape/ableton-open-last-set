...
def get_live_preferences_filepath(live_version=''):
    ...
    live_folders = [name for name in os.listdir(root_pref_folder) if os.path.isdir(os.path.join(root_pref_folder, name)) and live_folder_prefix in name]
    if not live_folders:
    ...
    # if a live version was specified via command args use that one, otherwise use the latest version
    if live_version and live_folder_prefix not in live_version:
        live_version = '{prefix}{version}'.format(prefix=live_folder_prefix, version=live_version)
    else:
        live_version = max(live_folders, key=parse_live_version_from_folder_name)
